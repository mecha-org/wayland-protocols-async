...
pub enum ToplevelMessage {
    GetToplevels {
        reply_to: oneshot::Sender<Vec<ToplevelKey>>,
    },
    GetToplevelMeta {
        key: ToplevelKey,
    ...
    Activate {
        key: ToplevelKey,
        reply_to: oneshot::Sender<Result<bool, ToplevelHandlerError>>,
    },
    SetFullscreen {
        key: ToplevelKey,
        reply_to: oneshot::Sender<Result<bool, ToplevelHandlerError>>,
...
fn map_state_to_wstate(state: Vec<u8>) -> Vec<ToplevelWState> {
    ...
    let is_maximized = state_iter
        .clone()
        .any(|s| s == zwlr_foreign_toplevel_handle_v1::State::Maximized as u32);
    let is_minimized = state_iter
        .clone()
        .any(|s| s == zwlr_foreign_toplevel_handle_v1::State::Minimized as u32);
    let is_activated = state_iter
        .clone()
        .any(|s| s == zwlr_foreign_toplevel_handle_v1::State::Activated as u32);
    let is_fullscreen = state_iter
        .clone()
        .any(|s| s == zwlr_foreign_toplevel_handle_v1::State::Fullscreen as u32);
...
impl ToplevelHandler {
    pub fn new(event_tx: Sender<ToplevelEvent>) -> Self {
        ...
        let (globals, mut event_queue) =
            globals::registry_queue_init::<ToplevelState>(&conn).unwrap();
        let qh = event_queue.handle();

        let _toplevel_manager = globals
            .bind::<ZwlrForeignToplevelManagerV1, _, _>(
                &qh,
                core::ops::RangeInclusive::new(1, 3),
                (),
            )
            .map_err(|_| "compositor does not implement foreign toplevel (v1..3).")
            .unwrap();
        ...
        ToplevelHandler { event_queue, state }
    }
            ...
            tokio::select! {
                ...
                msg = msg_rx.recv() => {
                    ...
                    match msg.unwrap() {
                        ...
                        ToplevelMessage::MinimizeAll { reply_to } => {
                            for (_, t) in toplevel_state.toplevels.iter() {
                                t._handle.set_minimized();
                            }
                            let _ = reply_to.send(Ok(true));
    ...
    fn event(
        ...
        match event {
            zwlr_foreign_toplevel_handle_v1::Event::Title { title } => {
                ...
                let toplevel = state.toplevels.get_mut(toplevel_key).unwrap();
                toplevel.title = title.clone();

                state.dispatch_event(ToplevelEvent::Title {
                    key: toplevel_key,
                    title,
                });
            }
            zwlr_foreign_toplevel_handle_v1::Event::AppId { app_id } => {
                ...
                state.dispatch_event(ToplevelEvent::AppId {
                    key: toplevel_key,
                    app_id,
                });
            }
            zwlr_foreign_toplevel_handle_v1::Event::State {
                ...
                state.dispatch_event(ToplevelEvent::State {
                    key: toplevel_key,
                    state: toplevel_state.clone(),
                });
            }
            zwlr_foreign_toplevel_handle_v1::Event::Done => {
                ...
                state.dispatch_event(ToplevelEvent::Done {
                    key: toplevel_key,
                    title,
                    app_id,
                    state: toplevel_state,
            ...
            zwlr_foreign_toplevel_handle_v1::Event::OutputEnter { output } => {
                let toplevel_key = key.lock().unwrap().unwrap();
                state.dispatch_event(ToplevelEvent::OutputEnter {
                    key: toplevel_key,
                    output,
                });
            }
            zwlr_foreign_toplevel_handle_v1::Event::OutputLeave { output } => {
                let toplevel_key = key.lock().unwrap().unwrap();
                state.dispatch_event(ToplevelEvent::OutputLeave {
                    key: toplevel_key,
                    output,
                });
            }
            zwlr_foreign_toplevel_handle_v1::Event::Parent { parent } => {
                let toplevel_key = key.lock().unwrap().unwrap();
                state.dispatch_event(ToplevelEvent::Parent {
                    key: toplevel_key,
                    parent,
                });
            }
